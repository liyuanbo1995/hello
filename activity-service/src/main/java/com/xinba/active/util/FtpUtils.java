...
public class FtpUtils {
	...
	public static Boolean loginToFtpServer() {
		String serverUrl = "39.98.164.160",
				userName = "ispftp",
				passWord = "123456";
	...
	/**
	 * 批量上传FTP文件服务器
	 * @param files 多个文件输入流，map对象形式传入
	 * @param serverPath FTP服务器路径
	 * @return true 批量上传成功，否则就失败
	 * @throws Exception
	 */
	public static boolean batchUpload(Map<String, InputStream> files,
									  String serverPath) throws Exception {
		if (files.keySet().size() == 0) {
			return false;
		}
		for (Map.Entry<String, InputStream> entry : files.entrySet()) {
			boolean result = uploadToServer(entry.getValue(), serverPath, entry.getKey());
			if (!result) {
				return false;
			}
		}
